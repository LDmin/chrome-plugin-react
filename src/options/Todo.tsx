...
import { Calendar, Card, Form, Input, Button, DatePicker, TimePicker } from 'antd'
...
export default () => {
  ...
  useEffect(() => {
    resetList(todos.map(todo => ({
      ...todo,
      disabled: todo.disabled || moment().isAfter(moment(`${todo.end} ${todo.time}`, `${DATE_FORMAT} ${TIME_FORMAT}`))
    })))
  }, [todos])

  const onFieldChange = useCallback((index: number, key: string, value: any) => {
    replace(index, {
      ...list[index],
      [key]: value,
    })
    ...
    },
    {
      ...
      render: (t, r, i) => (
        <DatePicker
          ...
          onChange={v => onFieldChange(i, 'start', v?.format(DATE_FORMAT) ?? '')}
        />
      )
    },
    {
