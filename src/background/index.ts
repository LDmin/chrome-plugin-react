...
function createTask<V extends TaskValue>(tasks: Tasks<V>, task: V) {
  const endms = moment(`${task.end} ${task.time}`, `${DATE_FORMAT} ${TIME_FORMAT}`).valueOf()
...
chrome.storage.onChanged.addListener((changes) => {
  if (changes.todos) {
    ...
    if (newValue) {
      // 更新和删除
      for (const k of Object.keys(todoTasks)) {
        const todo: Todo | undefined = newValue.find(t => t.id === k)
        if (todo) {
          // 更新值
          todoTasks[k].value = todo
        } else {
          // 删除todos里没存在的值
          clearTimeout(todoTasks[k].task)
          delete todoTasks[k]
        }
      }
      newValue.map(todo => {
